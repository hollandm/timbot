...
*/
int getAccel(int deviceI2CAddress, byte * dataLoc) {
  int error;
  ...
  if (error = MPURead(deviceI2CAddress, ACCEL_X_L, &dataLoc[0]) != SUCCESS){
    return error;
  }
  if (error = MPURead(deviceI2CAddress, ACCEL_X_H, &dataLoc[1]) != SUCCESS){
    return error;
  }
  
  if (error = MPURead(deviceI2CAddress, ACCEL_Y_L, &dataLoc[2]) != SUCCESS){
    return error;
  }
  if (error = MPURead(deviceI2CAddress, ACCEL_Y_H, &dataLoc[3]) != SUCCESS){
    return error;
  }
  
  if (error = MPURead(deviceI2CAddress, ACCEL_Z_L, &dataLoc[4]) != SUCCESS){
    return error;
  }
  if (error = MPURead(deviceI2CAddress, ACCEL_Z_H, &dataLoc[5]) != SUCCESS){
    return error;
  }
  return SUCCESS;
};
...
*/
int getGyro(int deviceI2CAddress, byte * dataLoc) {
  if (MPURead(deviceI2CAddress, GYRO_X_L, &dataLoc[0]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  if (MPURead(deviceI2CAddress, GYRO_X_H, &dataLoc[1]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  
  if (MPURead(deviceI2CAddress, GYRO_Y_L, &dataLoc[2]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  if (MPURead(deviceI2CAddress, GYRO_Y_H, &dataLoc[3]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  
  if (MPURead(deviceI2CAddress, GYRO_Z_L, &dataLoc[4]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  if (MPURead(deviceI2CAddress, GYRO_Z_H, &dataLoc[5]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  return SUCCESS;
};
...
*/
int getTemp(int deviceI2CAddress, byte * dataLoc) {
  if (MPURead(deviceI2CAddress, TEMP_L, &dataLoc[0]) != SUCCESS){
    return MPU_READ_ERROR;
  }
  if (MPURead(deviceI2CAddress, TEMP_H, &dataLoc[1]) != SUCCESS) {
    return MPU_READ_ERROR;
  }
  return SUCCESS;
};
...
*/
int MPURead(int deviceI2CAddress, int addr, byte * data){
  int numBytes = 1;
...
*/
int MPUWrite(int deviceI2CAddress, int addr, byte * data){
  Wire.beginTransmission(deviceI2CAddress);
  if (Wire.write(addr) != 1){
    return MPU_ADDR_REQ_ERROR;
  }
